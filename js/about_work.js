document.addEventListener("DOMContentLoaded", () => {
    // 設定する変数
    const jsonFilePath = "json/about_work.json";
    const displayContainer = document.querySelector("#work-container"); // 表示するリストのコンテナ
    const toggleButton = document.querySelector("[data-key='about.show-more-work']"); // 表示切り替えボタン
    ...
    function createListItem(item, lang) {
        ...
        listItem.classList.add("work-list-item");

        const position = item.position?.[lang] || ""; // 各プロパティが存在するかチェック
        const company = item.company?.[lang] || "";
